...
/** Pure: play with the pet. Returns a new PetState. */
export function playWithPet(pet: PetState, now: number): PetState {
  ...
  return {
    ...
    growthProgress: Math.min(1, pet.growthProgress + GROWTH_CARE_PER_ACTION.play),
    lastActiveAt: now,
  };
}
